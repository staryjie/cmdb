...
func SaveResource(ctx context.Context, tx *sql.Tx, base *resource.Base,
	...
	// 保存资源数据，因为IP信息是一个字符串列表，需要转换成字符串进行保存
	_, err = stmt.ExecContext(ctx,
		base.Id, base.ResourceType, base.Vendor, base.Region, base.Zone,
		base.CreateAt, info.ExpireAt, info.Category, info.Type, info.Name,
		info.Description, info.Status, info.UpdateAt, base.SyncAt,
		info.SyncAccount, info.PublicIPToString(), info.PrivateIPToString(),
		info.PayType, base.DescribeHash, base.ResourceHash, base.SecretId,
