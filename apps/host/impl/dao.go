...
import (
	...
	"github.com/staryjie/cmdb/apps/resource/impl"
)
...
func (s *service) save(ctx context.Context, h *host.Host) error {
	...
	defer func() {
		if err != nil {
			if err := tx.Rollback(); err != nil {
				s.log.Errorf("Tx Rollback error, %s", err)
			}
		} else {
			if err := tx.Commit(); err != nil {
				s.log.Errorf("Tx Commit error, %s", err)
			}
		}
	}()
	...
	_, err = stmt.ExecContext(ctx,
		h.Base.Id, desc.Cpu, desc.Memory, desc.GpuAmount, desc.GpuSpec,
		desc.OsType, desc.OsName, desc.SerialNumber, desc.ImageId,
		desc.InternetMaxBandwidthOut, desc.InternetMaxBandwidthIn,
		desc.KeyPairNameToString(), desc.SecurityGroupsToString(),
	)
